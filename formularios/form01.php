...
<body>
    ...
    $ultimoNumero = isset($_COOKIE['exec01']) ? $_COOKIE['exec01'] : '';
    
    // Detecta parâmetros GET para mostrar resultado
    $status = isset($_GET['status']) ? $_GET['status'] : '';
    $numeroExibido = isset($_GET['numero']) ? $_GET['numero'] : '';
    $mostrarResultado = !empty($status);
    ?>
    
    <div class="container">
        <h2>Exercício 1 - Verificação de Número</h2>
        
        <div class="descricao">
            Insira um número abaixo para verificar se ele é <strong>positivo</strong>, <strong>negativo</strong> ou <strong>zero</strong>.
        </div>
        
        <?php if ($mostrarResultado): ?>
            <div class="resultado-message resultado-<?php echo $status; ?>">
                O número <?php echo htmlspecialchars($numeroExibido); ?> é <?php echo $status; ?>!
            </div>
        <?php endif; ?>
        
        <form method="POST" action="../exercicios\exec01.php">
